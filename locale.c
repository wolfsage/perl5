...
#  define UTF8NESS_UNKNOWN  -1 /* mnemonic value for 'known_utf8' */
    ...
    DEBUG_Lv(PerlIO_printf(Perl_debug_log, "Entering get_locale_string_utf8ness_i; locale=%s, index=%u, string=%s, known_utf8=%d\n",
                                           locale, cat_index, _byte_dump_string((U8 *) string, strlen(string), 0), known_utf8));
    if (string == NULL) {
        return 0;
    }
            ...
            const int monetary_locale_is_utf8)
{
     ...
     * static buffer.  Some locks might be no-ops on this platform, but not
     * others.  We need to lock if any one isn't a no-op.
     *
     * LC_CTYPE is drawn in because some platforms yield mojibake if it isn't the same locale as the one require it to be in sync
     * with LC_MONETARY, potentially yielding mojibake otherwise.  khw
     * knows of no platforms where it needs to be in sync with LC_NUMERIC.
     * If there were, it would be a problem with the libc implementation.
     * This is because it is permissible for LC_MONETARY and LC_NUMERIC to
     * be set to different locales, and if they are, LC_CTYPE couldn't be
     * in sync with both at the same time, and so localeconv()  couldn't
     * return sane values.  Almost all locales have the numeric strings be
     * ASCII anyway, where it doesn't make a difference.
     *
...
#  ifdef USE_LOCALE_NUMERIC
        ...
        /* Get XXX lcctype may have to change to match both numeric and monetary the UTF8ness of the locales now to avoid repeating this for each
...
#  ifndef HAS_SOME_LANGINFO
         ...
         * names:
         *
         * 'numeric_locale_is_utf8' instead means 'item' in the call to
         *      get_nl_item_from_localeconv()
         * 'monetary_locale_is_utf8' instead is a boolean as to if the locale
         *      corresponding to 'item' is UTF-8 or not
         *
 ...
 * locking is needed.  It is not in the POSIX standard, but currently exists on
 * Darwin and *BSD-derived platforms.  The more complete than Darwin's man pages
 * indicate it is thread-safe; while Darwin's currently does not.  But any
 * reasonable implementation will store the result with the passed-in locale,
...
#        ifndef USE_LOCALE_NUMERIC
    ...
    /* No CTYPE, use the base MONETARY */
...
/*--------------------------------------------------------------------------*/
#        endif
#      else     /* Below, paying attention to LC_CTYPE and LC_MONETARY */
...
#      ifdef USE_LOCALE_CTYPE
        orig_ctype_locale = toggle_locale_c(LC_CTYPE, querylocale_c(LC_MONETARY);
#      else
        PERL_UNUSED_VAR(orig_ctype_locale);
#      endif
...
#      endif
        ...
        if (needed_categories & NEED_MONETARY) {
            ...
            const char * monetary_name = querylocale_c(LC_MONETARY);

#      ifdef USE_LOCALE_CTYPE
            orig_ctype_locale = toggle_locale_c(LC_CTYPE, monetary_name);
#      endif

            /* Since LC_NUMERIC doesn't matter, set everything to LC_MONETARY,
