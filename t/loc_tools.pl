...
sub _trylocale ($$$$) { # For use only by other functions in this file!
    ...
    foreach my $category ($master_category, $categories->@*) {
        ...
        if (    $dashless_locale !~ / ^ \Q$result\E /xi
...
# Windows boxes sometimes erroneously return that ".UTF-8" is a valid locale.
# So we can't try that on such platforms.
my $utf8_suffix = ($^O =~ / MSWin32 /xi)
                   ? "65001"
                   : "UTF-8";
